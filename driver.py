...
if platform.system() == 'Darwin':
    if os.path.exists("/Users/rajeevkumar/Downloads/chromedriver"):
        browser = webdriver.Chrome(executable_path="/Users/rajeevkumar/Downloads/chromedriver",
...
try:
    for index, url in enumerate(urls, start=0):
        ...
        serial = str(index + 1)
        ydl_opts = {'cookiefile': 'cookies.txt', 'force_generic_extractor': True, 'no_check_certificate': True, 'outtmpl': os.getcwd() + '/' + title + "/" + serial + '. ' + re.sub('[?/:]', '',
                                                                                          lessons[index]) + '.%(ext)s',
